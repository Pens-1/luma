...
async def run_aider_evolution_auto(instruction: str) -> str:
    """
    Aiderにファイルを自動選択させてコードを修正する（スケーラブル版）。
    
    Args:
        instruction (str): 修正指示（ファイル名不要）
    ...
    if not os.path.exists(os.path.join(cwd, ".git")):
        ...
        try:
            ...
            subprocess.run(["git", "config", "user.name", "LUMA Bot"], cwd=cwd, check=True)
            subprocess.run(["git", "add", "."], cwd=cwd, check=True)
            subprocess.run(["git", "commit", "-m", "Initial commit"], cwd=cwd, check=False)
        except Exception as e:
            return f"❌ Git initialization failed: {str(e)}"

    print(f"🚀 Running Aider with auto file detection...")
    
    # Aider実行（複数ファイルを候補として渡す）
    cmd = [
        "aider",
        "--model", "ollama/qwen3-coder:30b",
        "--yes",
        "--no-auto-commits",
        "--message", f"{instruction}。関連するファイルを自動で見つけて修正してください。",
        "app/plugins/*.py",  # 全プラグイン
        "app/services/*.py",  # 全サービス
        "app/bot.py"  # Botメインロジック
    ]
    ...
    try:
        ...
        return f"✅ Aider Auto-Evolution Completed!\n\n```\n{output[-800:]}\n```"
